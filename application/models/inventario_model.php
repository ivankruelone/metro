...
/////////////////////////////////////////////////////////////////////
    ...
    function control_c()
    {
        ...
        $this->db->select('a.*,b.susa,sum(a.cantidad)as canti' );
        $this->db->from('inventario_d a');
        $this->db->join('catalogo.cat_nvo_metro_sec b' , 'a.clave=b.clave_metro', "left");
        $this->db->where('a.cantidad<>0','',false);
        $this->db->group_by('a.clave');
        $this->db->order_by('b.susa');
        $query = $this->db->get();
...
function busca_lotess($clave,$id_cc)
	{
		$sql = "SELECT lote, caducidad,cantidad FROM metro.inventario_d where clave= ? and cantidad>0
        order by caducidad";
        ...
        foreach($query->result() as $row)
        {
            ...
            $tabla.="
            <option value =\"".$row->lote."\">".$row->caducidad." ".$row->lote." - $row->cantidad Pzas</option>
            ";
        }
        }
